...
TC3 Driver Create Route Successfully
    ...
    # --- สร้างการเดินทาง ---
    Wait Until Element Is Visible    xpath=//form[@id='postRouteForm']//button[@type='submit' and contains(.,'สร้างการเดินทาง')]    8s
    ${submit_btn}=    Get WebElement    xpath=//form[@id='postRouteForm']//button[@type='submit' and contains(.,'สร้างการเดินทาง')]
    ...
    # ตรวจสอบผลลัพธ์
    ${success_toast}=    Run Keyword And Return Status    Wait Until Page Contains    สำเร็จ    8s
    ${redirected}=       Run Keyword And Return Status    Wait Until Location Contains    /findTrip    8s
    IF    not ${success_toast} and not ${redirected}
        Fail    ไม่พบข้อความสำเร็จและไม่ redirect ไป /findTrip
    END
    Capture Page Screenshot
